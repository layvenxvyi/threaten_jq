...
class Config(object):
    #sqlalchemy的配置参数
    SQLALCHEMY_DATABASE_URI="mysql://数据库名:数据库密码@127.0.0.1:3306/表名"
    #设置sqlalchemy自动跟踪数据库
...
def getdata():
    arr1={'0':'默认全选','1':'阿里云','2':'腾讯云','3':'CNNVD','4':'绿盟'}
    arr2={'0':'默认全选','1':'操作系统','2':'中间件','3':'开发框架','4':'数据库','5':'web应用','6':'其他'}
    ...
    select_origin=request.form.get("select_origin")
    select_shotkey=request.form.getlist("select_shotkey")
    select_val_ori = request.args.get("select_val")
    select_val=ast.literal_eval(select_val_ori) if select_val_ori else {"select_type":select_type,"select_origin":select_origin,"select_shotkey":select_shotkey}
    pageid = request.args.get('page', 1, type=int)
    print(select_val)
    if select_val=={'select_type': None, 'select_origin': None, 'select_shotkey': []} or select_val=={'select_type': '0', 'select_origin': '0', 'select_shotkey': []}:
        ...
        return render_template('threaten.html', datas=event2, arr1=arr1, arr2=arr2, arr3=arr3, select_val=select_val)
    else:
    #返回筛选数据，不支持分页，太麻烦了
        event2,datasum=return_data(select_type,select_origin,select_shotkey)
        return render_template('threaten.html', datas=event2,arr1=arr1, arr2=arr2, arr3=arr3,select_val=select_val)
...
class updata_Threaten(object):#更新爬取数据的类
    ...
    def wx_Sendmessage(self,content,shotkey,shottype):#调用微信接口进行消息推送http://wxpusher.zjiecode.com/admin/app/list
        ...
        #需要自己注册一个，填入自己的apptoken和topicid号，url写自己的地址
        params = {"appToken": "请在此处填入自己注册的apptoken值",
...
def scheduler_func():
    ...
    try:
        #默认2小时进行一次爬取，判断是否有新增数据出现，可自行调节时间
        scheduler.add_job(func=updata_Threaten, trigger='cron', hour='*/2', id='start_update')
        scheduler.start()
    except:
        #执行脚本出错则微信推送告知本人
        params = {"appToken": "请在此处填入自己注册的apptoken值","content": '脚本运行出错了', "contentType": 1, "topicIds": [428], "uids": [], "url": ""}
        html = requests.post('http://wxpusher.zjiecode.com/api/send/message', data=params, headers={'Content-Type': 'application/json', })
        print(html)

if __name__=='__main__':
    scheduler_func()
    app.run(host='0.0.0.0',port=8888,debug=True,use_reloader=False)#debug模式默认会对程序再执行一次，所以后面要加use_reloader=Flase，默认端口5000
